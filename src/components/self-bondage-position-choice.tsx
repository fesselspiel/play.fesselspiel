...
export function SelfBondagePositionChoice({
  ...
  return (
    <section>
      <h2 className="mb-2 text-sm font-semibold text-graphite">Auftragsstellung</h2>
      ...
      <div className="space-y-2">
        ...
        <label className="block rounded-md bg-paper p-3 text-sm">
          <span className="flex items-center gap-3">
            <input
              ...
              className="h-4 w-4 accent-redbrand"
            />
            <span className="font-medium">Freitext statt Stellung</span>
          </span>
          {choice === "custom" ? (
        ...
        <label className="flex items-start gap-3 rounded-md bg-paper p-3 text-sm">
          ...
          <span>
            ...
            <span className="block text-xs text-graphite">Der Auftrag erlaubt eine beliebige passende Self-Bondage-Stellung.</span>
          </span>
        </label>
        {!positions.length ? (
          <p className="rounded-md bg-paper p-3 text-sm text-graphite">Es gibt noch keine Stellung mit dem Feld „Self-Bondage-fähig“. Nutze Freitext oder „Denk dir was aus“.</p>
        ) : null}
      </div>
    </section>
  );
