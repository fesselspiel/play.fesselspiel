...
export async function GET(request: NextRequest) {
  ...
  await getOrCreateCatalogCategory("position", auth.user.tenantId);
  const categories = await prisma.catalogCategory.findMany({
    where: {
      kind: "position",
      ...tenantWhere(auth.user.tenantId),
      name: { not: defaultCategoryNames.position }
    },
...
export async function POST(request: NextRequest) {
  ...
  const category = await getOrCreateCatalogCategory("position", auth.user.tenantId, name);
  await logAction({
    actorId: auth.user.id,
    action: "position_category_created_api",
    entityType: "catalog_category",
    entityId: category.id,
    title: `Szenen-Kategorie per API angelegt: ${category.name}`,
    href: "/settings/api-control",
    details: { kind: "position", name: category.name }
  });
