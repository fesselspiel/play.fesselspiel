...
export async function PATCH(request: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.catalogCategory.findFirst({
    where: { id: params.id, kind: "position", ...tenantWhere(auth.user.tenantId) }
  });
  ...
  try {
    ...
    await logAction({
      ...
      action: "position_category_updated_api",
      entityType: "catalog_category",
      entityId: category.id,
      title: `Szenen-Kategorie per API geändert: ${category.name}`,
      href: "/settings/api-control",
      details: { kind: "position", oldName: existing.name, name: category.name }
    });
