...
Tracker-Typen liefern und akzeptieren zusätzlich `quotaReminderSchedule`. Der Zeitplan enthält getrennte Regeln für `daily`, `weekly` und `monthly`, jeweils mit `enabled`, `startMinutes` und `repeatMinutes`. Wochenregeln enthalten außerdem `weekday` (0 Sonntag bis 6 Samstag), Monatsregeln `dayOfMonth` (1 bis 28 oder 0 für den letzten Monatstag). Tagesregeln erlauben 60 Minuten, 3, 6 und 12 Stunden sowie einmal täglich. Wochen- und Monatsregeln akzeptieren zusätzlich jeden ganzzahligen Tagesabstand bis 28 Tage; die App begrenzt Wochenregeln passend zur Periode auf 1 bis 7 Tage. `quotaReminderEnabled` bleibt der Hauptschalter pro Tracker; die bisherigen Intervallfelder bleiben für Rückwärtskompatibilität erhalten.
