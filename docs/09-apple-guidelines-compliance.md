...
- Vorher waren Tagebuch/Wiki und Ideensammlung zwei technisch getrennte Modelle. Frei benennbare Bereiche mit eigener Nutzer-/Zirkel-Freigabe fehlten. Die neuen Modelle `ContentSpace`, `ContentSpaceEntry`, `ContentSpaceUserShare` und `ContentSpaceCircleShare` sind rein additiv.
- Bestehende `WikiPage`- und `ActivityPlan(category=IDEA_COLLECTION)`-Datensaetze bleiben unveraendert. Zuordnungen referenzieren sie mit `sourceType` und stabiler `sourceId`; es findet keine Inhaltskopie und keine Sichtbarkeitserweiterung statt. Standardbereiche `Tagebuch` und `Ideen` werden idempotent erzeugt und vorhandene Inhalte mit `skipDuplicates` zugeordnet.
- Neue Bereichseintraege verwenden weiterhin die bewaehrte Wiki-Seite als Quelldatensatz. Dadurch bleiben Revisionen, Dateizugriff, Transkription, Kalenderdatum, Datenexport, UGC-Melden und Kontoloeschung kompatibel. Vorhandene Ideen bleiben als Legacy-Quelle voll editierbar.
- Sichtbarkeit ist explizit `PRIVATE`, `USERS`, `CIRCLES` oder `SHARED`. Nutzer- und Zirkel-IDs werden mandantenbezogen validiert. Blockierte Eigentuemer werden fuer normale Benutzer aus der Bereichssicht ausgeschlossen. Der bestehende `X-Playplaner-View-Context` wird ueber `requireApiUser` weiter respektiert.
- Das Archivieren eines benutzerdefinierten Bereichs loescht keine Inhalte: Wiki-Zuordnungen wechseln in `Tagebuch`, Ideen-Zuordnungen in `Ideen`. Standardbereiche koennen nicht geloescht werden. Ein explizites Entry-DELETE bleibt eine eigene Nutzeraktion.
- Beim Zirkel-Austritt werden eigene geteilte Bereiche auf privat gesetzt und betroffene Nutzer-/Zirkel-Freigaben entfernt. Datenexport enthaelt Bereichsmetadaten und Zuordnungen; die vorhandene echte Kontoloeschung entfernt sie durch User-Kaskaden.
- Produktionsmigration ist zweistufig und darf erst nach Backup erfolgen: `prisma/manual-migrations/20260714_content_spaces/up.sql`, danach `node scripts/content-spaces-backfill.js`. Rueckbau der idempotenten Standardzuordnung: `node scripts/content-spaces-backfill.js --rollback-defaults`; anschliessend kann `down.sql` die neuen Tabellen/Enums entfernen. Quelldaten in `WikiPage`, `ActivityPlan`, `FileAsset` und deren Relationen werden dabei nicht geloescht. Vor einem Rueckbau nach produktiver Nutzung sind benutzerdefinierte Bereichsnamen/Freigaben aus dem Datenexport zu sichern.
- Isolierter PostgreSQL-16-Nachweis: aktuelles Schema erstellt, `down.sql`, `up.sql` und erneut `down.sql` liefen mit `ON_ERROR_STOP=1`. Nach dem finalen Down waren `WikiPage` und `ActivityPlan` weiterhin vorhanden, `ContentSpace` nicht mehr. Der Backfill erzeugte beim ersten Lauf 2 Bereiche/2 Zuordnungen, beim zweiten 0/0; Rollback entfernte 2/2 und liess beide Quelldatensaetze bestehen.
- Ein frischer Next-15.5.20-Dockerbuild bestand mit allen neuen Routen. Ein isolierter HTTP-Smoke meldete sich nativ an, las zwei Standardbereiche, legte einen benutzerdefinierten Bereich und Eintrag an, aenderte Titel/Kalenderdatum, archivierte den Bereich, fand den Eintrag danach im Tagebuch und loeschte ihn explizit. Testcontainer und -datenbank wurden vollstaendig entfernt. Prisma-Generate, TypeScript und `COMPLIANCE_STATIC_OK` bestanden.
- Noch nicht als live markieren: Die Produktionsmigration und ein reversibler API-Smoke mit vorhandenen Legacy-Daten muessen nach dem Deploy durchgefuehrt werden. Die iOS-Oberflaeche folgt in einem getrennten Zyklus gegen diesen finalen Vertrag.
