...
- `src/lib/api-tokens.ts`: Bearer-Header bevorzugen und den vom Betreiber bewusst benoetigten URL-Token-Vertrag fuer Alexa, Browser und Kurzbefehle erhalten; Tokenwerte niemals automatisch in App-Links oder Download-URLs serialisieren.
...
| 2026-07-13 | Tokens | Automatisch erzeugte tokenhaltige Download-URLs entfernt; der spaeter erneut bestaetigte Betreibervertrag erlaubt URL-Tokens fuer externe Automationen | Clients bevorzugen Bearer; Alexa/Browser/Kurzbefehle duerfen `token` als Query senden; Serializer liefern weiterhin `downloadUrlWithToken: null` | Query-Fallback in `tokenFromRequest` entfernen; keine Datenmigration | TypeScript und statischer Compliance-Test; Transportarten getrennt dokumentiert | Erfuellt mit dokumentiertem URL-Risiko |
...
- Die Tokenverwaltung liefert einen neu erzeugten Token einmalig in einer `no-store`-Antwort. Bearer bleibt die bevorzugte Transportart. Der Betreiber benoetigt zusaetzlich URL-Tokens fuer Alexa, Browser und Kurzbefehle; die Verwaltung weist deshalb ausdruecklich auf Browser-Historie, Proxy- und Serverlogs hin. Backend-Serializer erzeugen weiterhin keine tokenhaltigen Download- oder Redirect-URLs.
- Die zentrale Passwortregel lässt den Benutzer jedes nicht-leere Passwort bis 128 Zeichen selbst wählen und gilt fuer Einladung, E-Mail-Bestaetigung, Reset, eigenes Passwort sowie administrative und externe Benutzerverwaltung. Die App erzwingt keine 12-Zeichen-Mindestlänge. Automatisch erzeugte Passwoerter bleiben kryptografisch zufaellig; BCrypt-Hashing und Rate-Limits bleiben unverändert.
- Rueckbau: Zuerst den zugehoerigen Code-Commit revertieren, danach `prisma/manual-migrations/20260714_security_rate_limits/down.sql` anwenden. Die Migration entfernt ausschliesslich kurzlebige Rate-Limit-Zaehler. Sie veraendert keine Konten, Passwoerter, Inhalte oder Tokens. Der spaetere URL-Token-Fallback kann unabhaengig davon durch Entfernen der Query-Zeilen in `tokenFromRequest` deaktiviert werden.
...
## Zyklus 21: konsistenter Token-Vertrag

- Bearer bleibt fuer native Apps, Downloads und die API-Konsole der kanonische Weg. Der Betreiber hat den zusaetzlichen Query-Token-Vertrag fuer Alexa, Browser und Kurzbefehle danach ausdruecklich wieder aktiviert; er gilt fuer alle Methoden, erzeugt aber keine tokenhaltigen Links in Serverantworten.
...
- Der statische Compliance-Test verlangt jetzt beide bewusst unterstuetzten Transportarten und einen sichtbaren Hinweis auf das URL-Leak-Risiko. Mobile-Dokumentation und API-Konsole bleiben Bearer-orientiert, damit Apps keine Tokens in URLs schreiben.
- Der historische Negativtest fuer Query-Token ist durch den expliziten Alexa-/Automationsvertrag ueberholt. Aktuelle Abnahme muss denselben aktiven Token per Bearer und Query jeweils erfolgreich pruefen; Multipart-Token im Request-Body bleibt ungueltig.
- Rueckbau: Text, Test und die Query-Fallback-Zeilen koennen ohne Migration gemeinsam revertiert werden. Multipart-Token im Body wird weiterhin nicht unterstuetzt.

## Zyklus 32: URL-Token fuer Alexa und externe Automationen wiederhergestellt

- Auf ausdrueckliche Betreiberanforderung akzeptiert `tokenFromRequest` wieder den Query-Parameter `token`, wenn kein Bearer-Header gesetzt ist. Der Fallback gilt fuer alle externen Methoden, damit bestehende Alexa-, Browser- und Shortcut-Integrationen unveraendert funktionieren.
- Bearer hat Vorrang. Die native iOS-App, API-Konsole, geschuetzte Dateiabrufe und serverseitig erzeugte URLs bleiben beim Header; `downloadUrlWithToken` bleibt `null`. Dadurch wird kein Token unbeabsichtigt von der App in Links kopiert.
- Die API-Token-Verwaltung erklaert beide Transportarten und warnt sichtbar vor Browser-Historie, Proxy- und Serverlogs. Wer eine URL mit Token teilt oder protokolliert, muss den Token anschliessend widerrufen und neu erzeugen.
- Rueckbau: In `src/lib/api-tokens.ts` die URL-Auswertung entfernen, Hinweise und Verifier auf Bearer-only zuruecksetzen. Keine Datenbankmigration und keine Tokenkonvertierung erforderlich.
...
- Bestehende `WikiPage`- und `ActivityPlan(category=IDEA_COLLECTION)`-Datensaetze bleiben unveraendert und werden ueber `scripts/content-spaces-backfill.js` in echte Standardbereiche `Tagebuch` und `Ideensammlung` gespiegelt. Die API-Aliase `legacy-wiki` und `legacy-ideas` loesen nur noch auf diese echten Bereiche auf.
...
- Das DELETE eines benutzerdefinierten Bereichs setzt nur `archivedAt`; Bereich, Eintraege und Anlagen bleiben fuer einen kontrollierten Rueckbau erhalten. Standardbereiche koennen geaendert, aber nicht geloescht werden. Ein explizites Entry-DELETE bleibt eine eigene Nutzeraktion.
...
- Rueckbau nach vorherigem Datenexport: `prisma/manual-migrations/20260714_content_spaces/down.sql` entfernt ausschliesslich die drei neuen Tabellen. `WikiPage`, `ActivityPlan` und deren Dateien bleiben bestehen. `up.sql` stellt die generischen Tabellen wieder her; der idempotente Backfill kann danach erneut ausgefuehrt werden.
...
- Ein frischer Next-15.5.20-Dockerbuild bestand mit allen Routen. Ein isolierter HTTP-Smoke meldete sich nativ an, las beide Standardbereiche, legte einen benutzerdefinierten Bereich und eigenen Eintrag an, aenderte Titel/Kalenderdatum und archivierte den Bereich. Danach war er nicht mehr aufrufbar, waehrend Bereich und Eintrag nachweislich in der Datenbank erhalten blieben. `SecurityRateLimit`, `LegalDocument` und `ContentReport` waren gleichzeitig vorhanden. Prisma-Generate, TypeScript und `COMPLIANCE_STATIC_OK` bestanden.
...
- Commit `bb62c4e` wurde nach dem geschuetzten Backup `/opt/kink-social-platform/backups/pre-content-spaces-reconcile-20260714-085259` produktiv ausgerollt. Ein reversibler Live-Smoke pruefte Standardbereiche, benutzerdefinierten Bereich, nativen Eintrag, Kalenderdatum, PNG-Anlage, Anlage-Loeschung, Archivierung und Tokenwiderruf. Alle temporaeren Datensaetze, Auditlogs und Dateien wurden anschliessend entfernt.
...
- Content-Space-Eintraege sind einheitlich als `contentEntry` meldbar. Der zentrale Report-Resolver prueft Zielzugriff und bestimmt den gemeldeten Urheber selbst. Migrierte Wiki- und Ideeninhalte bleiben ueber `sourceType/sourceId` nachvollziehbar, werden in der mobilen Content-Space-API aber nicht mehr als eigene Legacy-Reporttypen behandelt.
...
## 2026-07-15 - Native Medienpraeferenz und isolierte Seitenansicht

- `PATCH /api/external/account/privacy-settings` speichert `showSensitiveMedia` additiv und streng typisiert. Die native iOS-Ansicht kann damit nicht eingestufte private Bilder pro Benutzer unmittelbar ein- oder ausblenden; Freigaben werden nicht erweitert. `EXPLICIT` und `QUARANTINED` bleiben in iOS unabhaengig von dieser Praeferenz verborgen.
- Der reversible Produktions-Smoke aenderte die Einstellung kurzzeitig, las sie zurueck, stellte den Ursprungswert wieder her und verwarf unbekannte Felder. Ergebnis: `NATIVE_SENSITIVE_MEDIA_LIVE_OK changed=1 restored=1 unknown_rejected=1`. Vor dem Rollout liegt das Backup unter `/opt/kink-social-platform/backups/pre-native-sensitive-media-20260715-084701`.
...
## 2026-07-15 - TestFlight Build 114

- Der ausschliesslich ueber Fastlane erzeugte Build `1.0 (114)` wurde nach einem Uploadlimit-Preflight von `6/15` mit dem App-Store-Profil signiert und erfolgreich hochgeladen. App Store Connect meldet Build-ID `f7de58f8-a129-43b6-80b3-1792d34ead17`, `VALID`, intern `IN_BETA_TESTING`, extern `READY_FOR_BETA_SUBMISSION` und `usesNonExemptEncryption=false`.
