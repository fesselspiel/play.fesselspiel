...
- App Store Connect enthaelt Store-Texte, Support-/Privacy-URL, Lifestyle als Primaer- und Productivity als Sekundaerkategorie, ehrliche 18+-Altersdeklaration, Review-Kontakt, Review Notes, einen live geprueften Demo-Zugang, je drei neutrale iPhone- und iPad-Screenshots sowie 15 veroeffentlichte Privacy-Deklarationen. Build `1.0 (114)` ist `VALID`, intern `IN_BETA_TESTING`, export-compliance-bereinigt und einer internen All-Builds-Gruppe mit Tester zugeordnet; die vorbereitete Store-Version verwendet Build 114 und `AFTER_APPROVAL`. Es wurde keine Review-Einreichung erzeugt.
...
| 9 Authentifizierung/Einladungen | Erfuellt mit dokumentiertem URL-Risiko | Bearer bevorzugt; Query-Token fuer bewusst konfigurierte Alexa-/Browser-/Shortcut-Automationen, persistente Limits, Reset-/Invite-Schutz, Sitzungsverwaltung und Widerruf | URL-Tokens koennen in Verlauf und Infrastruktur-Logs erscheinen; keine automatische URL-Serialisierung |
...
Der Fastlane-Buildstatus bestaetigt Version `1.0`, Build `114`, `VALID`, `AFTER_APPROVAL`, keine erzeugte Submission, aufgeloeste Export-Compliance und interne Testverfuegbarkeit. Die Submit-Lane bleibt technisch gesperrt, bis eine lokale Datei mit Modus `600` exakt fuer Version 1.0/Build 114 neun getrennte reale Endabnahmen bestaetigt: Face ID, Privacy Overlay, Inaktivitaetssperre, Kernflows, Rechtstexte, besetzte Kontakte, Moderationsbetrieb, Drittanbieterangaben und Product Page.
