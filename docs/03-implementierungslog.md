...
- Der Rule-Editor unterstuetzt nun mehrere fachlich konfigurierte Aktionen pro Regel. Aktionen koennen ohne JSON hinzugefuegt, entfernt, je nach Geraetefaehigkeit eingeschraenkt, in der Zusammenfassung beschrieben, im Ablauf angezeigt und in der Simulation als mehrere wartende beziehungsweise faellige Aktionen sichtbar gemacht werden.
- Geraetefaehigkeiten besitzen in der normalen Administration nun typbezogene Konfigurationsfelder: Kamera mit Datenpunkt, Timeout, Bildalter und Boot-Wartezeit, Schalter mit Datenpunkt und Ein-/Aus-Werten sowie Sprachausgabe mit Datenpunkt und optionalem Praefix. Die Werte werden in den bestehenden `parametersJson`-Feldern gespeichert, ohne dass Admins JSON pflegen muessen.
- Die Automation-Session-Seite zeigt aktive Sessions nun mit pro-Session-Ereignisverlauf, eigener Historie und zustandsabhaengigen Aktionen. Bei vorgemerktem Ende wird das bestehende Zeitfenster klar erklaert und statt eines normalen Stop-Dialogs nur ein bewusster Override angeboten.
- Der fachliche Regelkatalog deckt nun zusaetzlich Session beendet, Bild empfangen, Kamera wieder verbunden, Kamera nicht erreichbar und allgemeine Capability-Ereignisse ab. Diese Ausloeser sind im Editor mit deutschen Labels verfuegbar und werden von der bestehenden Rule-Engine gematcht.
- Die Rule-Editor-Simulation zeigt nun explizit ausgelöste Regeln, wartende, fällige und simulierte Aktionen sowie ein vorgemerktes Ende. Damit ist der Scrubber näher an der fachlichen Abnahme, weil Pending-End und bereits simulierte Schritte direkt sichtbar sind und nicht nur in technischen Details stehen.
- Das Automation-Protokoll zeigt in der normalen Ansicht nur noch bekannte fachliche Detailfelder mit deutschen Beschriftungen. Interne oder unbekannte JSON-Felder bleiben vollständig im Aufklapper „Technische Details“, statt als rohe Keys in der Standardansicht aufzutauchen.
- Die Rule-Simulation wertet Geräte- und Capability-Bedingungen jetzt gegen den aktuellen simulierten Zustand aus. Passt eine Bedingung fachlich nicht, werden die betroffenen Aktionen als blockiert angezeigt und nicht mehr trotzdem als fällig simuliert.
- Auslöser wie Kamera nicht erreichbar, Kamera wieder verbunden, Bild empfangen, Capability-Ereignis und Gerätezustand können im Rule-Editor nun fachlich auf eine konkrete Fähigkeit beziehungsweise ein konkretes Gerät eingeschränkt werden. Die Rule-Engine respektiert diese Zielauswahl bei echten Events, statt solche Regeln global auf jedes passende Ereignis anzuwenden.
- Die Rule-Simulation zeigt relevante Variablen nun als eigene deutsche Box im Editor: Auslöser inklusive Ziel, Bedingung, aktuelles Bedingungsergebnis, Wartezeit/Zufallsfenster, Fälligkeit und den Hinweis, dass keine echten Side Effects ausgeführt werden. Die externe Simulations-API übernimmt dieselbe Trigger-Zielinformation.
...
- Bridge-Kommandos sind reconnect-fester: Geclaimte, aber nicht quittierte Geräteaktionen werden nach Ablauf einer Lease erneut bereitgestellt und protokolliert. Bereits terminale Adapter-Resultate werden idempotent zurückgegeben, ohne Erfolgs-/Fehlerereignisse doppelt zu erzeugen.
- Die Automation-Session-Seiten verwenden in der normalen Bedienung nun fachliche Sprache: `Actions` wurde zu `Geplante Aktionen`, leere Zustände sprechen von Aktionen und die vorzeitige Beendigung eines Pending-End-Fensters wird als `Sofort-Beendigung` statt als technischer Override erklärt.
- Die Rule-Editor-Simulation kann Geräte- und Fähigkeitszustände jetzt direkt in der Oberfläche überschreiben. Admins sehen dadurch ohne echte Bridge-Aktion, ob eine Online-/Offline- oder Zustandsbedingung erfüllt, blockiert oder noch offen ist; die Testwerte erscheinen zusätzlich in den deutschen Simulationsvariablen.
- Die externe Automation-Simulations-API unterstützt dieselben Testzustände über `simulationOverrides.deviceHealth` und `simulationOverrides.capabilityState`; IDs werden gegen die aktuelle Seite gefiltert, damit Web, App und Agent dieselbe geführte Simulation verwenden können.
- Sichtbare Automation-Oberflächen wurden weiter von Entwicklerbegriffen bereinigt: `Trigger`, `Bridge`, `Commands`, `Recovery` und `Rule-Editor` wurden in der normalen Bedienung durch Auslöser, Gerätebrücke, Gerätebefehle, Wiederherstellung und Regel-Editor ersetzt. Technische Rohdaten bleiben nur in technischen Aufklappern oder API-Pfaden sichtbar.
- Kamera-Fähigkeiten bieten im Regel-Editor nun zwei fachliche Aktionen: `Bild anfordern` erzeugt wie bisher einen geschützten Bildrequest mit optionaler Wiederherstellung, `Verbindung prüfen` erzeugt nur einen normalen Gerätebefehl an die Bridge und speichert kein Bild.
- Kamera-Bedingungen sind jetzt vollständig angebunden: `Letztes Kamerabild ist jünger als Vorgabe` kann ohne JSON im Regel-Editor konfiguriert, in der Timeline mit einem simulierten Bildalter geprüft, über die externe Simulations-API getestet und in der echten Rule-Engine gegen die zuletzt empfangenen geschützten Automation-Bildrequests ausgewertet werden.
- Schalter-Fähigkeiten unterstützen nun die Spezifikationsbedingung `ein/aus seit X Minuten`: Admins wählen im Rule-Editor den Schalter, den Zielzustand und die Dauer, simulieren Zustand plus Dauer im Scrubber und die echte Engine prüft den materialisierten Capability-State gegen dessen Änderungszeit.
- Schalter-Ereignisse sind nun eigene fachliche Auslöser: `Schalter wurde eingeschaltet`, `Schalter wurde ausgeschaltet` und `Schalter meldet Fehler` können im Rule-Editor direkt gewählt werden. Bridge-Resultate und externe Event-Meldungen materialisieren den Schalterzustand und erzeugen die passenden Automation-Events für Folge-Regeln.
- Sprachausgabe-Ereignisse sind nun ebenfalls eigene fachliche Auslöser: `Sprachausgabe wurde gestartet`, `Sprachausgabe wurde beendet` und `Sprachausgabe ist nicht erreichbar`. Bridge-Claims erzeugen den Start, Bridge-Resultate das Ende oder die Nicht-Erreichbarkeit; externe Event-Meldungen materialisieren den Voice-Zustand passend. Neu angelegte Voice-Fähigkeiten speichern die Aktion konsistent als `voice_speak`; vorhandene `speak`-Altwerte werden beim Start migriert.
- Regelbeschreibung, Ablaufdarstellung und Simulations-Timeline benennen Aktionen jetzt mit ihrem fachlichen Ziel. Aus `Bild anfordern` wird beispielsweise `Bild von Schlafzimmerkamera anfordern`; dadurch ist das Abnahmebeispiel im Editor ohne technische IDs nachvollziehbar.
- Die Regelliste und die kurze Simulationsübersicht berechnen ihre Beschreibung nun live aus der aktuellen Regeldefinition und den vorhandenen Geräten/Fähigkeiten. Dadurch bleiben auch ältere gespeicherte Regeln nach Verbesserungen der fachlichen Beschreibung verständlich, ohne dass historische Protokollversionen überschrieben werden.
- Die Simulationsboxen für wartende, fällige, simulierte und blockierte Aktionen zeigen nun pro Aktion einen fachlichen Detailtext mit Ziel, Fälligkeitsminute und relevanten Parametern wie Kamera-Timeout, Wiederholungen, Boot-Wartezeit oder Ansagetext. Damit bleiben auch Regeln mit mehreren Aktionen auf der Timeline nachvollziehbar.
- Die Automation-Einstellungen zeigen die Adapter-Erklärung weiterhin fachlich an, verschieben die konkreten API-Routen aber in einen geschlossenen Aufklapper `Technische API-Endpunkte`. Damit bleibt die normale Admin-Oberfläche verständlich, ohne die Integrationsdetails zu verstecken.
- Die Session-Detailseite nutzt für geplante Aktionen und Ereignisverweise dieselbe zielbezogene Aktionsbeschreibung wie Rule-Editor und Simulation. Statt nur `Bild anfordern` steht dort fachlich nachvollziehbar z. B. `Fordere ein Bild von Schlafzimmerkamera an`.
- Die Zielauswahl im Rule-Editor trennt Portal-Aktionen und Gerätefähigkeiten jetzt verständlicher. Statt der technischen Formulierung `Portal-Aktion ohne Gerät` steht dort `Session im Portal beenden`; Geräteziele sind als `Gerätefähigkeiten` gruppiert und ein Hilfetext erklärt, dass ohne Gerätefähigkeit nur diese Portal-Aktion möglich ist.
- Auslöser- und Bedingungs-Fähigkeiten im Rule-Editor sind nun fachlich nach Fähigkeitstypen gruppiert (`Kameras`, `Schalter`, `Sprachausgaben`). Ein Hilfetext erklärt, dass nach einem Auslöser nur die dazu passenden Fähigkeiten auswählbar sind; damit werden ungültige Kamera-/Schalter-/Sprachkombinationen bereits in der Oberfläche vermieden.
- Die Gerätebrücke zeigt in der normalen Admin-Ansicht nur noch Aktivierung und Verbindungszustand. MQTT-Themenbereich, Adapter-Client und Adapter-Benutzer liegen im Aufklapper `Verbindungsdetails` mit kurzem Hinweis, damit Adapter-Interna verfügbar bleiben, aber die Regeloberfläche nicht dominieren.
- Gespeicherte Regeln und die Simulationsübersicht verwenden nun dieselbe Flow-Vorschau mit roten Pfeilen wie der Rule-Editor. Dadurch bleibt der Ablauf `Auslöser -> Bedingung -> Zeitlogik -> Aktion` auch nach dem Speichern sofort wiedererkennbar.
- Automation-Protokolleinträge erhalten stabile Anker. Parent-/Child-Ereignisse im Abschnitt `Ursache und Folge` sind nun klickbar und springen direkt zum zugehörigen geladenen Ereignis, während technische IDs weiterhin nur im Aufklapper `Technische Details` stehen.
- Der Rule-Editor zeigt nun eine Inline-`Regelprüfung`, die dieselbe zentrale Validierung wie das Speichern nutzt. Administratoren sehen damit sofort, ob die Regel vollständig ist oder welche fachliche Auswahl noch fehlt, ohne erst speichern zu müssen.
- Die Simulation bietet zusätzlich zum Zeitregler Schnellpunkte für alle berechneten Timeline-Minuten. Administratoren können dadurch direkt zu Auslöser, Bedingung, Ende der Wartezeit, fälliger Aktion oder Wiederherstellung springen, ohne den Slider exakt treffen zu müssen.
- Die Simulation zeigt nun zusätzlich eine `Momentaufnahme bei Minute X`. Darin stehen die aktuell aktive Entscheidung, die zuletzt eingetretenen Schritte und die nächsten erwarteten Schritte in deutscher Sprache. Damit lässt sich beim Scrubben sofort nachvollziehen, warum eine Aktion noch wartet, blockiert ist oder fällig wird.
- Aktive Automationsgeräte zeigen ihre Fähigkeiten nun ebenfalls fachlich: pro Kamera, Schalter oder Sprachausgabe werden die daraus entstehenden Aktionen, Ereignisse und Bedingungen direkt in deutscher Sprache angezeigt. Technische Keys bleiben nur im Aufklapper `Technische Details`.
- Für die Simulation gibt es nun fachliche `Prüfpunkte`: Start, kurz vor der Bedingung, Bedingungsprüfung, frühester/mittlerer/spätester Zufallszeitpunkt, gezogener Zeitpunkt und nach der Fälligkeit. Das Abnahmebeispiel kann damit gezielt bei Minuten wie 19, 20, 24 und 30 geprüft werden, ohne den Slider exakt treffen zu müssen.
- Der Web-Start einer Automation-Session erinnert nun vor dem Start ausdrücklich an die unabhängige physische Not- oder Sicherheitsfreigabe. Der Startbutton kann erst genutzt werden, wenn bestätigt wurde, dass diese Freigabe unabhängig von Playplaner, Internet, MQTT, ioBroker und Stromversorgung eingerichtet und geprüft ist.
- Die Automation-Session-Detailseite zeigt Ereignisursachen und Folgeereignisse nun ebenfalls als klickbare Anker und stellt Policy-/Timing-Entscheidungen fachlich dar. Damit sind Live-Session-Verlauf und Admin-Protokoll konsistent: normale Benutzer sehen Ursache, Begründung, Rolle, erlaubte Aktion und geplante Ausführung; IDs, Payloads und Execution Context bleiben im technischen Aufklapper.
- Leere Bereiche der Rule-Simulation zeigen nun keine generische Platzhalterformulierung mehr. Stattdessen beschreibt jede Box fachlich, was der leere Zustand bedeutet, zum Beispiel dass keine Aktion fällig ist, keine Wiederherstellung geplant ist oder die Regel keinen Zufallswert nutzt.
- Beim Anlegen von Automationsgeräten werden logische IDs und Capability-Keys nicht mehr pro Fähigkeit wiederholt. Sie stehen nur noch in einem einzigen geschlossenen Aufklapper `Technische Details` mit kurzer Erklärung, während die normale Geräteanlage bei Name, Integration und fachlichen Fähigkeiten bleibt.
- Bildanforderungen materialisieren nun bei Auswahl einer Kamera-Fähigkeit automatisch auch das zugehörige Gerät und validieren beide gegen die aktuelle Seite. Automation-Übersicht und Session-Detailseite zeigen Bildstatus, Zeitpunkt, Kamera, Anforderer und Anlass fachlich an; geschützte Dateien laufen weiterhin über `FileAsset` und `/api/files/...`.
- Der Bereich `Simulation` in den Automation-Einstellungen zeigt nun jede gespeicherte Regel als eigene aufklappbare Simulation mit demselben interaktiven Regel-Editor und Scrubber. Dadurch lassen sich gespeicherte Regeln prüfen, ohne erst in die Bearbeitung zu wechseln; die alte Begrenzung auf eine reine Vorschau der ersten fünf Regeln entfällt.
- Die externe Automation-Rule-API liefert Regeln nun zusätzlich mit fachlicher `summary` und `flow` aus. Die Simulationsroute kann neben rohen Regelteilen auch eine gespeicherte `ruleId` simulieren und gibt dieselbe Zusammenfassung, denselben Ablauf und die scrubbable Simulation zurück, damit App, Agent und ioBroker dieselbe Regelinterpretation wie die Weboberfläche nutzen.
- Automation-Oberflächen haben nun zentrale Fallback-Texte für unbekannte Zustände, Quellen, Rollen, Integrationen und Aktionen. Dadurch rutschen neue oder externe technische Keys nicht mehr ungefiltert in normale Admin-Ansichten; unbekannte Ereignisse zeigen den gespeicherten fachlichen Titel, während die Rohwerte weiterhin unter `Technische Details` nachvollziehbar bleiben.
- Die Simulations-Prüfpunkte markieren bei zufälliger Verzögerung nun zusätzlich den Moment kurz vor dem frühesten Zufallszeitpunkt. Im Abnahmebeispiel kann ein Admin damit direkt Minute 24 anspringen und sehen, dass die Bedingung erfüllt ist, die Bildanforderung aber noch nicht fällig sein darf.
- Externe Automation-APIs serialisieren Sessions, Ereignisse, Actions und Bildanforderungen nun über `src/lib/external-automation-serializers.ts`. App, Agent und ioBroker erhalten damit zusätzlich zu technischen Details deutsche Labels, fachliche Zusammenfassungen, Pending-End-Informationen und geschützte Bildlinks über `/api/external/files/...`.
- Die Automation-Bildrequest-Endpunkte liefern nach Anlegen und Upload den vollständigen fachlichen Kontext inklusive Kamera/Gerät, Fähigkeit, Anforderer, Statuslabel und Datei-Metadaten zurück. Raw-/Payload-Daten bleiben unter `technicalDetails`, statt normale API-Konsumenten zu dominieren.
